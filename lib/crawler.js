  ...
  json.news.forEach(function (item, i) {
    ...
    request.get(item.image, { encoding: null }, function (err, res, data) {
      if (err || res.statusCode != 200 || data.length < 100) {
        ...
        if (++idx == length) {
          uploader.uploadData(image.encode('jpg'), 'mosaic/' + json.date + '.jpg'); //mosaic目录放聚合内容
        }
      }
    });
  });
...
var template = '\
  <div class="grid">\
    <div>\
      <img src="<%= config.domain + story.image %>" alt="<%= story.title %>"/>\
  ...
  request.get(url, function (err, res, data) {
    ...
    for (var i in images) {
      ...
      if (image.indexOf('_is.jpg') > 0) {
        item.body = item.body.replace(image, config.domain + '/avatar/' + getKey(image));
        if (isLatest)
          uploader.uploadLink(image, 'avatar/' + getKey(image));
      } else {
        item.body = item.body.replace(image, config.domain + '/content/' + getKey(image));
        if (isLatest)
    ...
    db.put(item, 'news-' + item.id).catch(function (err) {
    });
    if (isLatest)
      uploader.uploadData(JSON.stringify(item), 'story/' + id + '.json');
  });
}
function fetchBefore(beforeDate) {
  ...
  request.get(url, function (err, res, data) {
    ...
    db.put(json, 'daily-' + json.date).catch(function (err) {
    });
    if (initiated)
      uploader.uploadData(JSON.stringify(json), 'news/' + json.date + '.json');
    if (!initiated)
...
exports.run = function () {
  ...
  fetchSections();
  fetchLatest();
  setInterval(uploader.processQueue, 1000);
