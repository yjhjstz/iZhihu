...
router.get('/date', function (req, res, next) {
  if (moment(latestDate, 'YYYYMMDD').isValid()) {
    var startDate = moment(latestDate, 'YYYYMMDD').subtract('days', 1).format('YYYYMMDD');
    var endDate = moment(latestDate, 'YYYYMMDD').subtract('days', 30).format('YYYYMMDD');
  } else {
    var startDate = moment().utc().subtract('days', 1).format('YYYYMMDD');
    var endDate = moment().utc().subtract('days', 30).format('YYYYMMDD');
  }
  var news = [];
  ...
  db.get('daily-' + req.params.date).then(function (json) {
    ...
    db.get('daily-' + before).then(function (data) {
      json.news.push({
        ...
        thumbnail: '/thumbnail/' + data.date + '.jpg'
      });
...
// Restful API
router.get('/api/v1/news/latest', function (req, res, next) {
  res.json({news: crawler.latest().news});
});

router.get('/api/v1/news/hot', function (req, res, next) {
  res.json({title: '热门', news: crawler.hot().recent});
});

router.get('/api/v1/news/section', function (req, res, next) {
  res.json({title: '专题', sections: crawler.sections().data});
});

router.get('/api/v1/download/:file', function (req, res, next) {
  var fullpath = process.cwd() + '/download/' + req.params.file;
  //res.attachment(params.filename);
  res.download(fullpath, req.params.file, function(err){
    if (err) {
      console.log(err);
